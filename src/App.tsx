...
export default function App() {
    ...
    stationMap.forEach((pointMap, station) => {
      pointMap.forEach((data, point) => {
        ...
        if (data.distances.length > 0) {
          ...
          avgDistStr = (distSum / data.distances.length).toFixed(2);
        }
  ...
  const exportReducedPDF = () => {
    ...
    const tableData = reducedRows.map(row => {
      ...
      const d2 = parseFloat(row.distance2);
      let avgDistance = '-';
      if (!isNaN(d1) && !isNaN(d2)) avgDistance = ((d1 + d2) / 2).toFixed(2);
      else if (!isNaN(d1)) avgDistance = d1.toFixed(2);
      else if (!isNaN(d2)) avgDistance = d2.toFixed(2);
              ...
              <div className="divide-y divide-slate-100">
                {reducedRows.length > 0 ? (
                  reducedRows.map((row, index) => {
                    ...
                    if (!isNaN(d1) && !isNaN(d2)) {
                      avgDistance = ((d1 + d2) / 2).toFixed(2);
                    } else if (!isNaN(d1)) {
                      avgDistance = d1.toFixed(2);
                    } else if (!isNaN(d2)) {
                      avgDistance = d2.toFixed(2);
                    }
